...
// it: not the glove, not the gun. To start the fight you have to earn 2000
...
const ST = { SHELL: 'SHELL', SUMMONING: 'SUMMONING', CHASE: 'CHASE',
             SLAM: 'SLAM', SPIT: 'SPIT', DYING: 'DYING', DEAD: 'DEAD' };
...
export class EggBoss {
  ...
  get awake() { return this.state !== ST.SHELL && this.state !== ST.DEAD; }
  get sealed() { return this.state === ST.SHELL; }
  get shellRatio() { return this.shell / BOSS.shellHits; }
  get healthRatio() { return Math.max(0, this.health / BOSS.maxHealth); }
  // 0 → fresh, 1 → nearly dead. Drives every escalation.
  get rage() { return 1 - this.healthRatio; }

  _buildFallback() {
    const white = new THREE.MeshStandardMaterial({ color: 0xf7f2e6, roughness: .82, flatShading: true });
    const yolk  = new THREE.MeshStandardMaterial({ color: 0xf0a92c, roughness: .55, flatShading: true,
                                                   emissive: 0x996008, emissiveIntensity: 0.35 });
    this.fallback = new THREE.Group();

    const w = new THREE.Mesh(new THREE.SphereGeometry(BOSS.bodyRadius, 12, 8), white);
    w.scale.set(1, 0.42, 1);
    w.position.y = BOSS.bodyRadius * 0.42;
    w.castShadow = true;
    this.fallback.add(w);

    const y = new THREE.Mesh(new THREE.SphereGeometry(BOSS.bodyRadius * 0.42, 10, 8), yolk);
    y.scale.set(1, 0.6, 1);
  ...
  async _loadReal() {
    ...
    const m = await loadModel(EGG_URL, { maxSize: BOSS.size });
    if (!m) { console.warn('[boss] egg model unavailable, using fallback'); return; }
  ...
  _buildTelegraph() {
    this.tele = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: 0xe0392b, transparent: true, opacity: 0, side: THREE.DoubleSide })
    );
  ...
  hit(amount, worldPoint) {
    if (this.state === ST.SHELL) { this.hitFlash = 0.5; this.audio?.shellPing(); return 0; }
    ...
    if (this.weakPoint && worldPoint) {
      ...
      if (wp.distanceTo(worldPoint) < 1.6) { dmg *= BOSS.headshotMult; crit = true; }
    }
  ...
  raycast(origin, dir, maxDist = 60) {
    ...
    _v.copy(this.root.position).setY(this.root.position.y + BOSS.height * 0.42).sub(origin);
  ...
  update(dt, player) {
    ...
    switch (this.state) {
      ...
      case ST.SUMMONING: {
        ...
        if (this.stateTime > BOSS.summonTime) { this.state = ST.CHASE; this.stateTime = 0; }
        break;
      }
      ...
      case ST.CHASE: {
        ...
        const speed = THREE.MathUtils.lerp(BOSS.chaseSpeed, BOSS.enrageSpeed, this.rage);
        if (dist > 3.0) {
          to.normalize();
        ...
        if (dist < BOSS.slamRange && this.slamCd <= 0) {
          this.state = ST.SLAM; this.stateTime = 0;
        } else if (this.spitCd <= 0 && dist > 6 && this.rage > 0.2) {
          this.state = ST.SPIT; this.stateTime = 0;
      ...
      case ST.SLAM: {
        ...
        this.tele.material.opacity = Math.min(0.55, (this.stateTime / wind) * 0.55);
        ...
        if (this.stateTime >= wind) {
          ...
          if (d < BOSS.slamRadius) {
            ...
            this.onDamagePlayer?.(BOSS.slamDamage * (0.5 + falloff * 0.5), 'slam');
          }
          this.audio?.bossSlam();
      ...
      case ST.DYING: {
        ...
        this.body.rotation.z = Math.sin(t * 9) * 0.35 * Math.max(0, 1 - t / 2.4);
  ...
  _spit(player, spreadAngle) {
    ...
    const dir = to.clone().applyAxisAngle(new THREE.Vector3(0, 1, 0), spreadAngle);
    dir.y += 0.12 + dist * 0.006;
    dir.normalize();

    const mesh = new THREE.Mesh(
      new THREE.SphereGeometry(0.36, 8, 6),
      new THREE.MeshStandardMaterial({ color: 0xf0a92c, emissive: 0x995f08,
  ...
  _updateProjectiles(dt, player) {
    for (let i = this.projectiles.length - 1; i >= 0; i--) {
      ...
      const ground = this.world.groundHeight(p.mesh.position.x, p.mesh.position.z);
  ...
  _pose(dt) {
    ...
    this.body.position.y = Math.abs(Math.sin(this.bobT * 2.2 * speedish)) * 0.22 * jiggle;
    this.body.rotation.x = Math.sin(this.bobT * 2.2 * speedish) * 0.06 * jiggle
                         + this.wobble * Math.sin(this.bobT * 26) * 0.09;
    this.body.rotation.z = Math.sin(this.bobT * 1.7 * speedish) * 0.05 * jiggle
    ...
    if (this.state === ST.SLAM) {
      ...
      this.body.scale.setScalar(1 + this.crackT * 0.10);
    ...
    if (this.mats) {
      ...
      for (const m of this.mats) m.emissive.setRGB(f * 0.9, f * 0.12, f * 0.05);
    }
  }
}
