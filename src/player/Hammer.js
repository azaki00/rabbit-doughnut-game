...
// THE TENDERISER — bought for 1000c, and the only thing that cracks the shell.
//
...
export const HAMMER = {
  price: 1000,
  swingTime: 0.62,
  impactAt: 0.3, // seconds into the swing when the head lands
  cooldown: 0.25,
  range: 3.6,
...
// The tool itself, as geometry. Shared by the viewmodel and by the display
// piece turning over the cooking table, so the thing you buy is visibly the
// thing you are shown — if one changes, both do.
export function buildTenderiser() {
  ...
  const haft = new THREE.Mesh(
    new THREE.CylinderGeometry(0.021, 0.026, 0.52, 8),
    wood,
  );
  haft.position.set(0, -0.1, 0);
  tool.add(haft);
  ...
  const head = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.105, 0.2), steel);
  head.position.set(0, 0.17, 0);
  head.castShadow = true;
  tool.add(head);
  tool.userData.head = head;

  // the studs that make it a tenderiser rather than a sledge
  for (let ix = -1; ix <= 1; ix++) {
    for (let iz = -1; iz <= 1; iz++) {
      const stud = new THREE.Mesh(
        new THREE.BoxGeometry(0.021, 0.018, 0.021),
        steel,
      );
  ...
  const cheek = new THREE.Mesh(
    new THREE.BoxGeometry(0.055, 0.075, 0.075),
    steel,
  );
...
export class Hammer {
  ...
  _build() {
    this.tool = buildTenderiser();
    this.head = this.tool.userData.head;

    this.root.add(this.tool);
    this.root.traverse((o) => o.layers.set(LAYER_VIEWMODEL));

    this.root.scale.setScalar(0.85);
    this.restPos = new THREE.Vector3(0.24, -0.3, -0.42);
    this.restRot = new THREE.Euler(0.3, -0.3, 0.42);
  ...
  update(dt, input, active) {
    ...
    if (this.swing > 0) {
      ...
      if (
        !this.didImpact &&
        prev < HAMMER.impactAt &&
        this.swing >= HAMMER.impactAt
      ) {
        this.didImpact = true;
        this._impact();
      }
